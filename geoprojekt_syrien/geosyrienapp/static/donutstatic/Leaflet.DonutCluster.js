...
(function (factory, window) {
    ...
    function donut(options) {
        var div = doc.createElement('div'),
            ...
            colors = options.colors || ['#FE2E2E'], //Standart ist hier 555
    ...
    function createDonut(points, opt, cfgFn) {
		...
		// Hacker Magic- Umschreiben der Formel, damit mehr Werte ausgelesen werden können als einer
        for (var i = 0; i < count; i++) {
            var s = points[i].options[key];
            for(var j in points[i].options[key])
            {
                var x = points[i].options[key][j];
                if (!blocks[x]) 
                {
                    if (points[i].options[x]!=0)
                    {
                        //console.log(points[i].options[x]);
                        blocks[x] = 0;
                    }
                ...
                if (!sumField)
                {
                    if (points[i].options[x]!=0)
                    {
                        blocks[x]=blocks[x]+points[i].options[x];
                    }
                }
                else blocks[x] += points[i].options[sumField];
            }
            /*
		...
		// Einstellen der Dicke des Kreises 
	...
	// Summenfunktion angepasst, dass Zahlen addiert werden
    function defaultStyle(points) {
        //console.log(points);
        var sum = 0;
        for (var i in points)
        {
            sum+=points[i].options.sum;
        }
